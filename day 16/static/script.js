document.addEventListener("DOMContentLoaded", () => {
    ...
    // This is the single unified function for what happens after audio finishes playing
    const onAudioEnded = () => {
        statusDisplay.textContent = "Click the microphone to record again.";
        recordBtn.disabled = false;
    };

    // Event listener for the AI's response audio is set once
    ...
    const handleStopRecording = async () => {
        ...
        try {
            ...
            if (response.ok && !isError) {
                ...
                addMessageToChat(result.user_query_text, 'user-message');
                addMessageToChat(result.llm_response_text, 'agent-message');
                statusDisplay.textContent = "Here is my response:";
                audioPlayer.src = result.audio_url;
                audioPlayer.play();
                // No need to add audioPlayer.onended listener here, it's already set once.
            } else {
                statusDisplay.textContent = `❌ Server Error: I'm having trouble connecting right now.`;
                addMessageToChat("I'm having trouble connecting right now.", 'agent-message');
                
                const audioBlob = await response.blob();
                audioPlayer.src = URL.createObjectURL(audioBlob);
                audioPlayer.play();
                // No need to add audioPlayer.onended listener here
            }
        } catch (error) {
            console.error("Error with conversational agent:", error);
            addMessageToChat("❌ Network Error: Could not connect to the server.", 'agent-message');
    ...
    // Initial state setup
    statusDisplay.textContent = "Click the microphone to begin.";
});